...
def test(net,loader_test,max_psnr,max_ssim,step):
	...
	for i ,(inputs,targets) in enumerate(loader_test):
		...
		pred=net(inputs)
		# print(pred)
		tfs.ToPILImage()(torch.squeeze(targets[0].cpu())).save('111.png')
		vutils.save_image(targets[0].cpu(),'target.png')
		vutils.save_image(pred[0].cpu(),'pred.png')
		ssim1=ssim(pred,targets).item()
		psnr1=psnr(pred,targets)
		ssims.append(ssim1)
		psnrs.append(psnr1)
		if (psnr1>max_psnr or ssim1 > max_ssim) and s :
				ts=vutils.make_grid([torch.squeeze(inputs[0].cpu()),torch.squeeze(targets[0].cpu()),torch.squeeze(pred[0].clamp(0,1).cpu())])
